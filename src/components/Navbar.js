        ...
        <div ref={ref} className="overflow-y-scroll">
            <nav className={`fixed top-0 left-0 bg-[rgb(var(--navbar-color))] w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 shadow-md text-gray-700 py-3 md:py-4`}>
                ...
                <div className="hidden md:flex items-center gap-4 lg:gap-8">
                    {navLinks.map((link, i) => (
                        <Link key={i} href={link.path} className={`group flex flex-col gap-0.5 text-[rgb(var(--brand-green))]`}>
                            {link.name}
                            <div className={`bg-[rgb(var(--brand-green))] h-0.5 w-0 group-hover:w-full transition-all duration-300`} />
                        </Link>
                    ))}
                </div>
                ...
                <div className="hidden md:flex items-center gap-4">
                    <button className={`px-8 py-2.5 rounded-full ml-4 cursor-pointer transition-all duration-500 text-white bg-[rgb(var(--brand-green))] hover:bg-[rgb(var(--btn-hover))]`} onClick={()=> router.push('/login')}>
                        Login
                    </button>
                </div>
